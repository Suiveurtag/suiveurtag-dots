...
LATENCY_SAMPLES="${SPEEDTEST_LATENCY_SAMPLES:-3}"
DOWNLOAD_CHUNKS="${SPEEDTEST_DOWNLOAD_CHUNKS:-5}"
DOWNLOAD_BYTES_PER_CHUNK="${SPEEDTEST_DOWNLOAD_BYTES_PER_CHUNK:-2500000}"
UPLOAD_CHUNKS="${SPEEDTEST_UPLOAD_CHUNKS:-4}"
UPLOAD_BYTES_PER_CHUNK="${SPEEDTEST_UPLOAD_BYTES_PER_CHUNK:-1000000}"
...
emit_status "running" "latency" "0.02" "0" "" "" "" "Preparing latency samples" >/dev/null

latency_total=0
latency=""

for ((i = 1; i <= LATENCY_SAMPLES; i++)); do
    progress="$(awk -v i="$i" -v total="$LATENCY_SAMPLES" 'BEGIN { printf "%.2f", 0.02 + (i - 1) / total * 0.08 }')"
    emit_status "running" "latency" "$progress" "0" "" "" "$latency" "Ping sample $i/$LATENCY_SAMPLES" >/dev/null
    ...
    if [[ -z "$latency_sample" ]]; then
        finish_error "latency test failed"
    fi

    latency_total="$(awk -v total="$latency_total" -v sample="$latency_sample" 'BEGIN { printf "%.2f", total + sample }')"
    latency="$(awk -v total="$latency_total" -v count="$i" 'BEGIN { printf "%.0f", total / count }')"
    progress="$(awk -v i="$i" -v total="$LATENCY_SAMPLES" 'BEGIN { printf "%.2f", 0.02 + i / total * 0.08 }')"
    emit_status "running" "latency" "$progress" "0" "" "" "$latency" "Ping ${latency} ms" >/dev/null
done
...
for ((i = 1; i <= DOWNLOAD_CHUNKS; i++)); do
    progress="$(awk -v i="$i" -v total="$DOWNLOAD_CHUNKS" 'BEGIN { printf "%.2f", 0.12 + (i - 1) / total * 0.48 }')"
    ...
    progress="$(awk -v i="$i" -v total="$DOWNLOAD_CHUNKS" 'BEGIN { printf "%.2f", 0.12 + i / total * 0.48 }')"
...
for ((i = 1; i <= UPLOAD_CHUNKS; i++)); do
    progress="$(awk -v i="$i" -v total="$UPLOAD_CHUNKS" 'BEGIN { printf "%.2f", 0.64 + (i - 1) / total * 0.32 }')"
    ...
    progress="$(awk -v i="$i" -v total="$UPLOAD_CHUNKS" 'BEGIN { printf "%.2f", 0.64 + i / total * 0.32 }')"
