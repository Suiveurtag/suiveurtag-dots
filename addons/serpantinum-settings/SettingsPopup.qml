...
Item {
    ...
    function scrollHighlightedIntoView() {
        ...
        if (root.currentTab === 0 && generalLoader.item) {
            generalLoader.item.scrollToBox(box);
    ...
    // The recovered pre-V2 panel keeps its original layout and controls. Only
    // the panel's scope is narrowed to addon toggles, keybinds, and monitors.
    property var tabNames: ["Addons", "Keybinds", "Monitors"]
    property var tabIcons: ["󰏘", "󰌌", "󰍹"]
    property var tabColors: ["teal", "peach", "green"]
    property var tabValues: [0, 2, 3]
    property int visualTabIndex: root.currentTab === 0 ? 0 : (root.currentTab === 2 ? 1 : 2)
    ...
    Keys.onTabPressed: (event) => {
        ...
        let index = root.tabValues.indexOf(root.currentTab);
        root.currentTab = root.tabValues[(index + 1) % root.tabValues.length];
        event.accepted = true;
    }
    Keys.onBacktabPressed: (event) => {
        if (root.isSearchMode) return;
        let index = root.tabValues.indexOf(root.currentTab);
        root.currentTab = root.tabValues[(index - 1 + root.tabValues.length) % root.tabValues.length];
        event.accepted = true;
    }
    ...
    function handleRootEnter(event) {
        ...
        if (root.currentTab === 2) root.saveAllKeybinds();
    ...
    /* Serpantinum V2 exposes the same palette through ThemeBackend. */
    ...
    Component {
        id: addonsTabComponent
        Item {
            Loader {
                anchors.fill: parent
                source: "AddonSettingsPage.qml"
                onLoaded: {
                    item.uiScale = Qt.binding(function() { return 1; });
                    item.highlightedBox = Qt.binding(function() { return root.highlightedBox; });
                    item.settingsPath = Qt.binding(function() { return Quickshell.env("HOME") + "/.config/serpantinum/settings.json"; });
                    item.tealColor = Qt.binding(function() { return root.teal; });
                    item.blueColor = Qt.binding(function() { return root.blue; });
                    item.mauveColor = Qt.binding(function() { return root.mauve; });
                    item.sapphireColor = Qt.binding(function() { return root.sapphire; });
                    item.baseColor = Qt.binding(function() { return root.base; });
                    item.textColor = Qt.binding(function() { return root.text; });
                    item.subtextColor = Qt.binding(function() { return root.subtext0; });
                    item.surface0Color = Qt.binding(function() { return root.surface0; });
                    item.surface1Color = Qt.binding(function() { return root.surface1; });
                    item.surface2Color = Qt.binding(function() { return root.surface2; });
                    item.selected.connect(function(index) { root.highlightedBox = index; });
                }
            }
        }
    }
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                if (root.currentTab === 3) Config.applyMonitors();
                            }
                        }
                    }
                    ...
                    Flickable {
                        ...
                        Rectangle {
                            ...
                            property color targetColor: {
                                if (root.visualTabIndex === 0) return c0;
                                if (root.visualTabIndex === 1) return c1;
                                return c2;
                            }
                            color: targetColor
                            ...
                            property int curTab: root.visualTabIndex
                        ...
                        Row {
                            ...
                            Repeater {
                                ...
                                Item {
                                    ...
                                    property bool isActive: root.currentTab === root.tabValues[index]
                                    ...
                                    MouseArea {
                                        ...
                                        hoverEnabled: true
                                        cursorShape: Qt.PointingHandCursor
                                        onClicked: { root.currentTab = root.tabValues[index]; root.clearHighlight(); }
                                    }
                                }
                            }
                        }
                ...
                Item {
                    ...
                    Loader {
                        ...
                        sourceComponent: addonsTabComponent
