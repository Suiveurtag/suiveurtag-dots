...
Rectangle {
    ...
    readonly property string applyScript: dataHome + "/quickshell-addons/topbar-button-effects/apply.sh"

    function toggle() {
        if (applying) return;
        const requestedState = !animationsEnabled;
        animationsEnabled = requestedState;
        applying = true;
    ...
    RowLayout {
        ...
        ColumnLayout {
            ...
            Text {
                text: "Smooth press feedback and flowing Matugen outlines"
                font.family: "Inter"
        ...
        Rectangle {
            ...
            Rectangle {
                ...
                radius: root.s(8)
                y: root.s(3)
                x: root.animationsEnabled ? root.s(21) : root.s(3)
                color: root.animationsEnabled
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    root.animationsEnabled = settings.topbarButtonAnimations !== false;
                } catch (error) {
    ...
    Process {
        ...
        onExited: exitCode => {
            root.applying = false;
            if (exitCode !== 0) root.animationsEnabled = !root.animationsEnabled;
